-- ============================================================
--  FLUENT UI LIBRARY + LOADING SCREEN by akbarkidds
--  Loading screen muncul duluan, fade out sebelum window tampil
-- ============================================================
...
local ParentScreen   = RunService:IsStudio() and LocalPlayer.PlayerGui or game:GetService("CoreGui")
...
local function CreateLoadingScreen()
	...
	lsGui.Name          = "LoadingScreen"
	lsGui.ResetOnSpawn  = false
	lsGui.IgnoreGuiInset = true
	lsGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	lsGui.DisplayOrder  = 9999
	lsGui.Parent        = ParentScreen
	...
	container.Position          = UDim2.new(0.5,-290,0.5,-160)
	...
	local function makeDivider(yOffset)
		...
		d.Size              = UDim2.new(0,0,0,1)
		d.Position          = UDim2.new(0,0,0,yOffset)
		d.BackgroundColor3  = Color3.fromRGB(96,205,255)
		d.BorderSizePixel   = 0
		d.Parent            = container
		local g = Instance.new("UIGradient")
		g.Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0,   Color3.fromRGB(10,10,10)),
			ColorSequenceKeypoint.new(0.25, Color3.fromRGB(96,205,255)),
			ColorSequenceKeypoint.new(0.5,  Color3.fromRGB(167,139,250)),
			ColorSequenceKeypoint.new(0.75, Color3.fromRGB(244,114,182)),
	...
	local div1 = makeDivider(0)

	-- -- "by akbarkidds" label --------------------------------
	local byClip = Instance.new("Frame")
	byClip.Size               = UDim2.new(1,0,0,90)
	byClip.Position           = UDim2.new(0,0,0,18)
	byClip.BackgroundTransparency = 1
	byClip.ClipsDescendants   = true
	byClip.Parent             = container

	local byLine = Instance.new("TextLabel")
	byLine.Size               = UDim2.new(1,0,1,0)
	byLine.Position           = UDim2.new(0,0,1,0)   -- mulai dari bawah
	byLine.BackgroundTransparency = 1
	byLine.Text               = "X-GENERATION"
	byLine.Font               = Enum.Font.GothamBlack
	byLine.TextSize           = 52
	byLine.TextColor3         = Color3.fromRGB(96,205,255)
	byLine.TextXAlignment     = Enum.TextXAlignment.Center
	byLine.TextTransparency   = 1
	byLine.Parent             = byClip

	-- Gradient teks: biru ? ungu ? pink ? hijau
	local tg = Instance.new("UIGradient")
	tg.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0,    Color3.fromRGB(96,205,255)),
		ColorSequenceKeypoint.new(0.33, Color3.fromRGB(167,139,250)),
		ColorSequenceKeypoint.new(0.66, Color3.fromRGB(244,114,182)),
		ColorSequenceKeypoint.new(1,    Color3.fromRGB(52,211,153)),
	})
	tg.Parent = byLine

	local div2 = makeDivider(118)

	-- -- Status rows ------------------------------------------
	local statusData = {
		{ text = "INITIALIZING COMPONENTS", color = Color3.fromRGB(96,205,255)  },
		{ text = "LOADING ASSETS",          color = Color3.fromRGB(167,139,250) },
		{ text = "BUILDING INTERFACE",      color = Color3.fromRGB(244,114,182) },
		{ text = "READY",                   color = Color3.fromRGB(52,211,153)  },
	}
	local statusRows = {}
	for i, data in ipairs(statusData) do
		local row = Instance.new("CanvasGroup")
		row.Size               = UDim2.new(1,0,0,24)
		row.Position           = UDim2.new(-0.05,0,0,136+(i-1)*28)
		row.BackgroundTransparency = 1
		row.GroupTransparency = 1
		row.Parent             = container

		local dot = Instance.new("CanvasGroup")
	...
	-- -- Version line -----------------------------------------
	local verClip = Instance.new("Frame")
	verClip.Size               = UDim2.new(1,0,0,20)
	verClip.Position           = UDim2.new(0,0,0,256)
	verClip.BackgroundTransparency = 1
	verClip.ClipsDescendants   = true
	verClip.Parent             = container

	local verLabel = Instance.new("TextLabel")
	verLabel.Size              = UDim2.new(1,0,1,0)
	verLabel.Position          = UDim2.new(0,0,1,0)
	verLabel.BackgroundTransparency = 1
	verLabel.Text              = "VERSION 1.0.0  �  2025"
	verLabel.Font              = Enum.Font.Code
	verLabel.TextSize          = 12
	verLabel.TextColor3        = Color3.fromRGB(96,205,255)
	verLabel.TextXAlignment    = Enum.TextXAlignment.Center
	verLabel.TextTransparency  = 1
	verLabel.Parent            = verClip
	...
	-- -- TweenInfo presets ------------------------------------
	local TI_FAST = TweenInfo.new(0.6, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
	local TI_MED  = TweenInfo.new(0.5, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
	local TI_DIV  = TweenInfo.new(0.7, Enum.EasingStyle.Quad,  Enum.EasingDirection.Out)
	...
	local function runAnim()
		...
		-- Divider 1
		task.spawn(function()
			local div1Tween = TweenService:Create(div1, TI_DIV, {
				Size = UDim2.new(1,0,0,1)
			})

			div1Tween:Play()
			div1Tween.Completed:Wait()
		end)
		local div2Tween = TweenService:Create(div2, TI_DIV, {
			Size = UDim2.new(1,0,0,1)
		})
		div2Tween:Play()
		div2Tween.Completed:Wait()
		task.wait(0.75)
		-- "by akbarkidds" slide up + fade in
		local Title = TweenService:Create(byLine, TI_FAST, {
			Position         = UDim2.new(0,0,0,0),
			TextTransparency = 0,
		})
		Title:Play()
		Title.Completed:Wait()
		task.wait(1.75)

		-- Status rows
		for i, row in ipairs(statusRows) do
			task.delay((i-1) * 0.12, function()
				TweenService:Create(row, TI_MED, {
					GroupTransparency = 0,
					Position = UDim2.new(0,0,0,row.Position.Y.Offset),
				}):Play()
			end)
		end
		...
		-- Version line
		TweenService:Create(verLabel, TI_FAST, {
			Position         = UDim2.new(0,0,0,0),
			TextTransparency = 0,
		}):Play()
	end

	-- -- Fungsi publik: jalankan & tunggu selesai -------------
	function ls:PlayAndWait()
		runAnim()
		-- Total durasi animasi masuk � 3 detik
		task.wait(2)
	end

	-- -- Fungsi publik: fade out lalu destroy -----------------
	function ls:FadeOut(callback)
...
-- ============================================================
--  JALANKAN LOADING SCREEN SEBELUM LIBRARY DIMUAT
-- ============================================================
...
Components.Element = function(Title, Desc, Parent, Hover)
	local Element = {}
...
Elements.Button.__type = "Button"
function Elements.Button:New(Config)
...
Elements.Colorpicker.__type = "Colorpicker"
function Elements.Colorpicker:New(Idx, Config)
		...
		Creator.AddSignal(HexInput.Input.FocusLost, function(Enter)
			if Enter then
				local Success, Result = pcall(function()
					return Color3.fromHex(HexInput.Input.Text)
				end)
				if Success and typeof(Result) == "Color3" then
					Hue, Sat, Vib = Result:ToHSV()
				end
			end
...
Elements.Dropdown.__type = "Dropdown"
function Elements.Dropdown:New(Idx, Config)
	...
	local function RecalculateListSize()
		local itemHeight = 32 + 3 -- tinggi button + padding UIListLayout
		local maxItems = 5
		local totalItems = #Dropdown.Values

		local visibleItems = math.min(totalItems, maxItems)
		local listHeight = (visibleItems * itemHeight) + 43 -- 43 = search box (38) + padding (5)

		DropdownHolderCanvas.Size = UDim2.fromOffset(ListSizeX, listHeight)
	end
...
Elements.Input.__type = "Input"

function Elements.Input:New(Idx, Config)
	local Library = self.Library
...
Elements.Keybind.__type = "Keybind"
function Elements.Keybind:New(Idx, Config)
	local Library = self.Library
...
Elements.Paragraph.__type = "Paragraph"
function Elements.Paragraph:New(Idx, Config)
	local Library = self.Library
	assert(Config.Title, "Paragraph - Missing Title")
	Config.Content = Config.Content or ""

	local Paragraph = Components.Element(Config.Title, Config.Content, self.Container, false)
...
Elements.Slider.__type = "Slider"
function Elements.Slider:New(Idx, Config)
	local Library = self.Library
	...
	-- TextLabel untuk display (tidak ganggu drag)
	...
	-- TextBox input manual, muncul saat display diklik
	...
	-- Drag dari dot
	-- Ganti SliderDot.InputBegan ke HitBox
	...
	Creator.AddSignal(HitBox.InputEnded, function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or
			Input.UserInputType == Enum.UserInputType.Touch then
			Dragging = false
		end
	...
	-- Drag dari SliderInner (klik langsung di rail)
	...
	-- Klik display = munculkan input box
	SliderDisplay.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or
			Input.UserInputType == Enum.UserInputType.Touch then
			Dragging = false
	...
	-- Blokir karakter selain angka, titik, minus
	InputBox:GetPropertyChangedSignal("Text"):Connect(function()
		local Clean = InputBox.Text:gsub("[^%d%.%-]", "")
		if Clean ~= InputBox.Text then
			InputBox.Text = Clean
		end
	end)
	...
	function Slider:SetValue(Value)
		local Rounded = math.round(Value / Slider.Rounding) * Slider.Rounding
		self.Value = math.clamp(Rounded, Slider.Min, Slider.Max)
...
Elements.Toggle.__type = "Toggle"
function Elements.Toggle:New(Idx, Config)
	local Library = self.Library
...
-- Ganti dengan:
ElementsTable = ElementsTable or {}
table.insert(ElementsTable, Elements.Button)
table.insert(ElementsTable, Elements.Colorpicker)
table.insert(ElementsTable, Elements.Dropdown)
table.insert(ElementsTable, Elements.Input)
table.insert(ElementsTable, Elements.Keybind)
table.insert(ElementsTable, Elements.Paragraph)
table.insert(ElementsTable, Elements.Slider)
table.insert(ElementsTable, Elements.Toggle)
...
Components.TitleBar = function(Config)
	...
	TitleBar.CloseButton = BarButton(Components.Assets.Close, UDim2.new(1, -4, 0, 4), TitleBar.Frame, function()
		Library.Window:Dialog({
			Title = "Thank You for using XGen!",
...
function Notification:New(Config)
	...
	function NewNotification:Open()
		...
		RootMotor:setGoal({
			Scale = Spring.new(0, { frequency = 5 }),
			Offset = Spring.new(0, { frequency = 5 }),
		})
	end
	...
	function NewNotification:Close()
		if not NewNotification.Closed then
			...
			task.spawn(function()
				RootMotor:setGoal({
					Scale = Spring.new(1, { frequency = 5 }),
					Offset = Spring.new(60, { frequency = 5 }),
				})
				task.wait(0.4)
				if Library.UseAcrylic then
...
Components.Window = function(Config)
	...
	local Window = {
		...
		Position = UDim2.fromOffset(
			(Camera.ViewportSize.X - Config.Size.X.Offset) / 2,
			(Camera.ViewportSize.Y - Config.Size.Y.Offset) / 2
		),
	}

	local Dragging, DragInput, MousePos, StartPos = false, false, false, false
	local Resizing, ResizePos = false, false
	...
	local ToggleIconMobile = function(windowVisible, icon)
		...
		if not screenGui then
			...
			screenGui.Parent = ParentScreenx
			...
			corner.CornerRadius = UDim.new(1, 0) -- FULL BULAT
			...
			Window.Root.AncestryChanged:Connect(function(_, parent)
				if parent == nil then
					local screenGui = ParentScreen:FindFirstChild("MobileToggleGui")
					if screenGui then
						screenGui:Destroy()
					end
				end
...
local NotificationModule = Notification

local ProtectGui = protectgui or (syn and syn.protect_gui) or (gethui and function(gui) gui.Parent = gethui()end) or function() end
...
function Library:Round(Number, Rounding)
    local Result = math.round(Number / Rounding) * Rounding or 1
    local Decimals = #(tostring(Rounding):match("%.(%d+)$") or "")
...
function Library:CreateWindow(Config)
	...
	-- ?? PASTIKAN ELEMENTS SUDAH ADA DI LIBRARY
	self.Elements = self.Elements or Elements
	...
	-- ?? WINDOW HARUS PUNYA REFERENCE KE LIBRARY
	Window.Library = self
